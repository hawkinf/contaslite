  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.category), tooltip: 'Tabelas', onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AccountTypesScreen()))),
          IconButton(icon: const Icon(Icons.settings), tooltip: 'Config', onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsScreen()))),
        ],
      ),
...
class CreditCardItemWidget extends StatefulWidget {
  ...
  const CreditCardItemWidget({super.key, required this.card, required this.onUpdateNeeded});
...
class _CreditCardItemWidgetState extends State<CreditCardItemWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    detailsChildren.add(Text('PREVISÃO FATURA', style: TextStyle(color: fgColor.withOpacity(0.8), fontSize: 10, fontWeight: FontWeight.bold)));
    ...
    return Card(
      elevation: 4, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), margin: const EdgeInsets.symmetric(vertical: 8, horizontal: 4), child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), gradient: LinearGradient(colors: [bgColor, bgColor.withOpacity(0.8)], begin: Alignment.topLeft, end: Alignment.bottomRight)), padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            IconButton(icon: Icon(Icons.list_alt, color: fgColor, size: 24), tooltip: 'Ver Despesas', onPressed: () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => CardExpensesScreen(card: widget.card, month: _viewDate.month, year: _viewDate.year))); _loadTotals(); }), 
