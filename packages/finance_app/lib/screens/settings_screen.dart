...
class _SettingsScreenState extends State<SettingsScreen> {
  String _selectedRegion = PrefsService.regionNotifier.value;
  String _selectedCity = PrefsService.cityNotifier.value;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Configurações & Sobre')),
      body: ListView(
        ...
        children: [
            ...
            child: Column(
              children: [
                Container(
                    ...
                    child: const Icon(Icons.account_balance_wallet, size: 60, color: Colors.indigo)
                ),
          ...
          // --- APARÊNCIA ---
          const Text('Aparência', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.indigo)),
          SwitchListTile(
          ...
          // --- REGIÃO ---
          const Text('Localização (Feriados)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.indigo)),
          const SizedBox(height: 10),
          DropdownButtonFormField<String>(
            initialValue: _selectedRegion,
            decoration: const InputDecoration(labelText: 'Região', border: OutlineInputBorder()),
            items: HolidayService.regions.keys.map((r) => DropdownMenuItem(value: r, child: Text(r))).toList(),
            onChanged: (val) {
              setState(() {
                _selectedRegion = val!;
                _selectedCity = HolidayService.regions[val]!.first;
              });
              PrefsService.saveLocation(_selectedRegion, _selectedCity);
            },
          ),
          const SizedBox(height: 16),
          DropdownButtonFormField<String>(
            ...
            items: HolidayService.regions[_selectedRegion]!.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
            onChanged: (val) {
          ...
          ListTile(
            ...
            title: const Text('Apagar Todo o Banco de Dados', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
  ...
  Future<void> _showRecreateTablesDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Recriar Tabelas Padrão?', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
          ...
          FilledButton(
            ...
            onPressed: () async {
              if (confirmController.text.trim() == 'SIM') {
                ...
                try {
                  // Apaga tipos e categorias existentes
                  final db = DatabaseHelper.instance;
                  final types = await db.readAllTypes();
                  ...
                  // Apaga formas de pagamento
                  final methods = await db.readPaymentMethods(onlyActive: false);
                  for (final method in methods) {
                    await db.deletePaymentMethod(method.id!);
                  }
                  
                  // Recria as tabelas padrão
  ...
  Future<void> _showWipeDatabaseDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('TEM CERTEZA?', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
          ...
          FilledButton(
            ...
            onPressed: () async {
              if (confirmController.text.trim() == 'SIM') {
                ...
                // Feedback visual de erro ou apenas fecha (abortado)
                if (mounted) {
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
